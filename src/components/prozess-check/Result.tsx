  ...
  return (
    <div className="space-y-8">
      <div className="card-depth rounded-2xl border border-faden bg-lesepapier p-6 md:p-10">
        ...
        {copy.fits ? (
          <>
            {/* Pain first, at full size: the visitor's own number, then the same
                number as work days per year, then the honest arithmetic note. */}
            <p className="mt-3 text-balance font-serif text-4xl text-tiefes-wasser md:text-5xl">{copy.headline}</p>
            <p className="mt-2 text-pretty font-serif text-xl text-tiefes-wasser md:text-2xl">{copy.sub}</p>
            {copy.yearLine ? <p className="mt-4 text-pretty text-lg font-medium text-ember">{copy.yearLine}</p> : null}
            {copy.basis ? <p className="mt-2 text-pretty text-sm text-stumm">{copy.basis}</p> : null}
          </>
        ) : (
          <>
        ...
        {copy.topAreas.length > 0 ? (
          <div className="mt-6">
            ...
            <ul className="mt-3 space-y-3">
              {copy.topAreas.map((a) => (
                <li key={a.id} className="rounded-lg border border-faden bg-papier/70 p-4">
      ...
      {copy.fits ? (
        <div className="rounded-2xl bg-tiefes-wasser p-8 md:p-10">
          <p className="text-balance font-serif text-2xl text-papier md:text-3xl">{RESULT_UI.reliefTitle}</p>
          <p className="mt-3 text-pretty text-gletscher">{RESULT_UI.schedulerPrompt}</p>
          <div className="mt-6">
            <SchedulerEmbed
              calLink={calLink}
              prompt=""
