...
All six roadmap phases plus the post-Phase-6 polish are merged and deployed. Since launch the work has been **copy + design refinement** (see *Next session* / *Changelog* below). ⚠ The **2026-06-15 landing-page polish is committed on `feat/landing-page-merak-polish` — not yet merged/deployed.** The main blocker to go-live is **owner cutover** (domain, env vars, legal sign-off, Resend) — see the TODO blocks below.
...
6. **End-stage design pass** — the `RippleImage` WebGL water panel (degrades to a still on reduced-motion / no-WebGL; since moved off the hero, see *design system*); **`BrandLockup`** logo in Header + Footer; full-screen **`MobileNav`** drawer; petrol palette rhythm (`WasIchBaue` + `Steps` are `tone="petrol"`; *tiefes-wasser* reserved for Hero + Footer); favicon `src/app/icon.svg`.
...
> Homepage copy + hero are done (`feat/landing-page-merak-polish`) — **merge/deploy that branch** when ready. The end-of-page section order is a parked decision (Ideas.md #8).
...
4. **Cleanup (optional):** `/video/hero-quelle.jpg` is now unused (its replacement `was-ich-baue.webp` is the cleaner image and now carries the ripple). Remove or keep-for-reuse — decide later.
...
> - **2026-06-15 — landing-page polish** *(committed on `feat/landing-page-merak-polish`; not merged/deployed)*: full homepage copy + image pass via Hormozi `landing-page-copy`/`hormozi-hooks` (brand-filtered; outputs in `Knowledge/marketing/{LANDING_PAGE,HOOKS}.md`). Hero → pain-first **hours** H1 + warm *Merak* "work done" photo (`hero-merak.webp`) with CSS zoom+dust (`RippleImage` moved to `WasIchBaue`); CTA default → „Ruhe gewinnen“; `Problem` expanded to a 4-task list; `WasIchBaue` eyebrow dropped + ripple; `Steps` low-effort lead-in; `Proof` trust-by-logic; `GeschichteTeaser` trimmed to mention+link. Section-alignment done; end-of-page order deferred (Ideas.md #8).
