...
All six roadmap phases plus the post-Phase-6 polish are merged and deployed. Since launch the work has been **copy + design refinement** (see *Next session* / *Recently shipped* below). The main blocker to go-live is **owner cutover** (domain, env vars, legal sign-off, Resend) — see the TODO blocks below.
...
1. **Ratgeber seed articles** — verify the 3 seed drafts, then flip `draft: false` to publish (that's what makes the per-article covers visible in prod). Files: `content/ratgeber/*.mdx`.
...
3. **Design-skill pass on the remaining subpages** — Leistungen/Über-mich/FAQ are **done**; remaining: **Ratgeber (index + article), Kontakt, Newsletter, legal** (taste → high-end-visual-design → impeccable, redesign-preserve; browser-verify 1440/390 + AA).
4. **Replace „Flickenteppich“** — swap it for a clearer German term in `src/app/leistungen/page.tsx` (lead: „Kein Flickenteppich aus zehn Tools …“) and `src/components/home/WasIchBaue.tsx` (heading: „Eine saubere Quelle – kein Flickenteppich.“).
5. **Ratgeber authoring skill** — a custom skill that scaffolds a Ratgeber MDX article end-to-end: frontmatter (incl. **required `cover` + `coverAlt`**), BrandWord-aware body in the brand voice, and the matching cover-image prompt from `image_prompt.md`.
6. **Kontakt form go-live** — set the Resend env + Cal link (see Owner cutover), then verify `ContactForm` end-to-end (Server Action → Resend) + spam guards + success/error states live.
