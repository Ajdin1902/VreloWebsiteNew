...
- **Water imagery (home + pages)** — cohesive Nano Banana water photography (brand water world, cool-dominant), below the fold (Hero stays LCP), German alt text, browser-verified. Prompts in [image_prompt.md](image_prompt.md); optimized WebP in `public/images/`.
  - **Foreground (`next/image`):** Home `GeschichteTeaser` 2-col with the Bosnian karst spring; `WasIchBaue` 2-col with a portrait drop panel. `PageIntro` gained an **optional `image` prop** (`{ src, alt, ratio }` → `next/image fill` in a rounded `.shadow-deepwater` panel) driving banners on `/leistungen` (21:9), `/ueber-mich` (16:9), `/kontakt` (21:9), `/ratgeber` (16:9), `/faq` (16:9), `/newsletter` (16:9).
...
- **Ratgeber per-article covers (only imagery left):** wire the 4 ready cover variants — add a `cover` frontmatter field + render in `ArticleHeader` and the index cards. Source PNGs in the local `Images/` folder; convert PNG→WebP into `public/images/`. (All other home/page imagery is shipped.)
