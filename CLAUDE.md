...
## Resume here (Phases 1–5 shipped; Phase 6 built on a branch → merge + deploy)
**Phase 6 is built and gate-green on `feat/design-polish`** (all six planned build phases now exist) but **not yet merged/deployed**. Next: finish the branch → merge to `main` → push (Vercel auto-deploys) → live smoke check. After that, only **owner cutover** remains.

**Next steps:**
1. **Merge + deploy Phase 6** — merge `feat/design-polish` → `main`, push to deploy. Then verify live: Hero ripple follows the cursor and the drop seeds a ripple; mobile drawer opens / Esc / scroll-lock; petrol bands legible; favicon shows; OS reduced-motion falls back to the still image. (Plan Task 16 lists the full manual checklist.)
...
3. **Phase 6 backlog nits (non-blocking, from final review):** `MobileNav` does initial-focus + Esc + focus-return but is not a full Tab focus-trap (acceptable v1); the Hero panel keeps one inline `shadow-[…rgba]` (neutral, defensible).
