...
## Resume here (Phases 1–6 shipped + hero refinement → only owner cutover remains)
**All planned build work is merged + deployed**, including Phase 6 and the post-Phase-6 hero refinement (real drop-footage frame + centered warm-bloom drop). The only remaining work is **owner cutover** (domain, env vars, legal sign-off, Resend) — see the TODO blocks below. No code phase is queued.
...
2. **Live smoke check (optional):** after a deploy, confirm the Hero water frame + centered bloom render, the cursor ripple + center-seed work, the mobile drawer opens/Esc/scroll-locks, and OS reduced-motion falls back to the still frame.
