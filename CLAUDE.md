...
### In flight — `/makler` (branch `feat/makler-landingpage`, NOT merged)
A focus-mode outreach landing page for independent Makler / Finanz- und Versicherungsmakler: sells Die Termin-Quelle (lead, links to `/demo`) then the Document Concierge (second step), closing on the embedded Cal scheduler. Price-free; guarantee + Gründungs-Zusage on-page. Spec + plan: `docs/superpowers/{specs,plans}/2026-07-23-makler-landingpage*`. Copy source of truth stays in HQ (`Knowledge/Offers/Termin-Quelle.md`, `Knowledge/Strategy/Document_Concierge.md`).
- **Gate passed at `7910c3b`:** 490 tests, tsc/lint/build clean, browser-verified at *true* 390 and 1440. SSR HTML confirms `/`, `/lead-check`, `/demo` keep full chrome while `/makler` has none.
- **Final-review fixes applied** (`7910c3b`): guarantee intro narrowed to the *Schlussrechnung* (the offer takes a 50 % Anzahlung, so „du zahlst nichts im Voraus“ was false) · Document-Concierge trust copy moved to the **VreloVPS standard** (engine + upload page on the client's own server → no file ever touches Vrelo; the „Daten meiner Kunden“ objection follows) · `-mt-24 md:-mt-32` on `ProblemSection`/`TerminQuelleBlock` · `hero.ctaShort` + a responsive header-label split.
- ☐ **Open:** merge → then add `/makler` to the live route list above and point the outreach sequences at it (HQ `CLAUDE.md` §7).
- ☐ **Observation, not yet decided:** the `Bridge` beat sits in ~192px of space on each side at 1440 — its neighbours' `py-24/32` still double up around its deliberately tight `py-12/16`. Tightening means `-mt-*` on `Bridge` and `DocumentConciergeBlock`; founder has not seen it yet.
