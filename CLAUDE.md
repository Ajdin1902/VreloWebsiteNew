...
> - **2026-06-27 — Lead-Reaktions-Check (`/lead-check`)** (built on `feat/lead-reaktions-check`; deploys on merge to main): interactive `noindex` lead-magnet — full detail in the *design system* entry above. Pure `leadCheck.ts` model (TDD) + client wizard + optional-email Server Action (server-recompute). 279 tests green, browser-verified 1440/390.
