...
> - **2026-06-22 — dark reading-rhythm + gold form cards** (deployed): gave Über-mich/Leistungen/FAQ a petrol/papier light-dark rhythm; homepage Problem → `card-depth` panel, Proof → value cards; Newsletter/Kontakt forms moved into a new reusable **`WaterSection`** (petrol water) and their cards recolored navy → **`bg-amber` gold** with on-light text (teal+gold; picked from in-browser variants — honig/sonnenlicht/papier/gletscher were the alternatives). Added `--color-signal-tief` (on-light error) + `lightLinkClass`; `onDark` variants on StoryBeat/LeistungDetail/FaqItem. Specs/plans `docs/superpowers/{specs,plans}/2026-06-22-*`. Commits 3a75f0e…HEAD.
