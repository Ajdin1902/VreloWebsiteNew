...
- **Hero:** warm *Merak* "work done" photo (`/images/hero-merak.webp`, `next/image fill priority` = LCP) with gentle CSS life — a slow ambient zoom (`.hero-scene-img` / `hero-pan`) + 5 drifting dust motes (`.hero-dust` / `dust-float`); decorative `alt=""`, all motion-safe-gated. Hero text reveal is **H1 transform-only** so LCP paints immediately. *(The old `RippleImage` WebGL panel is retired from the hero — component still in repo, unused, available for reuse.)*
...
5. **Cleanup (optional):** `/video/hero-quelle.jpg` is now unused (the planned move into `WasIchBaue` was dropped — its existing `was-ich-baue.webp` is the cleaner, on-aspect image). `RippleImage.tsx`/`.test.tsx` are unused too. Remove or keep-for-reuse — decide later.
