...
**Bildwelt — geprüft 2026-08-06, kein Label im Bild nötig.** ⚠️ **Die ganze Bildwelt der Seite ist KI-generiert** — alle `PageHero`-Banner, der Home-Hero, die Section-Texturen, alle 12 Ratgeber-Cover (Nano Banana / Gemini 2.5 Flash Image) **und die vier Über-mich-Videos** `quelle · ripples · fluss · merak` (Seedance / Veo). Prompts + Herkunft: [image_prompt.md](image_prompt.md). Trotzdem **keine** Pflicht nach Art. 50 Abs. 4a: der Deepfake-Begriff (Art. 3 Nr. 60) verlangt kumulativ, dass der Inhalt **wirklichen** Personen/Orten/Ereignissen *ähnelt* **und** fälschlich als echt erscheint — abstraktes Wasser ohne erkennbares Motiv erfüllt das nicht, und Menschen kommt in keinem Prompt vor (universelles Negativ). **Abs. 2** (maschinenlesbare Markierung) trifft den **Anbieter** = Google, nicht uns; ⚠️ unsere `sharp`/ffmpeg-Konvertierung strippt Datei-Metadaten (SynthID ist pixelbasiert und übersteht das weitgehend, C2PA nicht) — also **nie behaupten**, die Markierung sei intakt.
- **Trotzdem gebaut:** ein Transparenzsatz auf `/ueber-mich`, direkt unter dem ersten Story-Beat (`bildhinweis` in `src/lib/ueber-mich.ts`, gerendert in `page.tsx`, test-guarded in `ueber-mich.test.ts` + `ueber-mich/page.test.tsx`). Grund ist **nicht** die KI-VO, sondern **UWG §5**: der Hero und der `quelle`-Clip zeigen eine „bosnische Karstquelle“ direkt neben Ajdins Ich-Erzählung „Mein Name kommt aus Bosnien und Herzegowina … wenn ich dort bin“ — genau die Nähe lässt einen Leser das Bild für ein echtes Foto *seiner* Quelle halten. Deshalb steht der Satz **an dieser Stelle**, nicht im Footer.
- **Zweite, seitenweite Stelle: `Bildnachweis` im Impressum** (`src/lib/legal/impressum.ts`, test-guarded in `legal.test.ts`). Der Über-mich-Satz deckt die Risikostelle, das Impressum den Rest der Seite (Home-Hero, `/leistungen`, 12 Ratgeber-Cover) — und kostet null Conversion, weil niemand das Impressum vor dem CTA liest. ⚠️ **Es nennt die Werkzeuge namentlich** (Gemini · Seedance/Veo): wer die Bildwelt mit einem anderen Generator neu würfelt, muss diesen Satz mitziehen.
- **Warum der Hinweis nicht ans Seitenende wandert** (gefragt und entschieden 2026-08-06): unten stünde er direkt vor `ClosingCta` — ein Zweifel-Beat genau im Moment des Asks (Value Equation: senkt Perceived Likelihood) — und er käme rechtlich zu spät, weil der falsche Eindruck oben bei der Ich-Erzählung entsteht. Nicht erneut vorschlagen.
- ⚠️ **Was Pflicht auslöst, bleibt:** realistische KI-Bilder/Videos, die reale Personen, Orte oder Ereignisse zeigen — dann Label **im Bild selbst** (z. B. „KI-generiert“ in einer Ecke), eine Bildunterschrift oder ein Kommentar reicht ausdrücklich nicht. Der Auslöser ist der **erkennbar reale Bezug**, nicht „Werbung ja/nein“ (die Website *ist* Werbung; sie ist nur gegenstandslos abstrakt). Gilt für jedes künftige Ad-Creative.
- **Ein echtes Foto bleibt ein echtes Foto:** es gibt bis heute **kein Founder-Porträt** auf der Seite; kommt eins, wird es fotografiert, nicht generiert (Notiz in `image_prompt.md` §6).
...
- **Page shape:** Hero (one-sentence lead, no CTA) → Problem → Termin-Quelle → mid-page CTA band (`MidCta`, the only in-page CTA before the close) → Document Concierge → „Was du dazu brauchst“ (`Voraussetzungen`) → Warum ich → Garantie → Einwände → Termin. Lists are headline-only; the page shows, it does not explain.
...
- **Interaktive Termin-Quelle-Demo (`/demo`):** role-reversal sandbox – the broker describes his business (optionally via an SSRF-guarded URL fetch + Haiku summary), then plays his own client against a **Claude-Haiku** booking bot; ends on a reveal that **closes on an inline booking** (2026-08-01). **A simulation, not the real n8n engine.** Pure-core in `src/lib/demo/` + three `nodejs`-runtime routes; **config-gated** (no `ANTHROPIC_API_KEY` → calm „bald verfügbar“ card). The bot runs a 7-step flow (name + e-mail asked last, once the Termin stands) and closes on a literal **`[ENDE]` sentinel**; `MAX_TURNS` (8) is the server-authoritative backstop. The reveal POSTs the transcript to the **fail-safe** `/demo/summary` route → a „Terminnotiz“ card + a simulated „Bestätigungsmail (Vorschau)“ – **display-only, nothing stored or sent** (matches the Datenschutz stance); any error → `EMPTY_NOTIZ` 200, never 5xx. **Reveal-Schluss (2026-08-01):** die Karte **scrollt sich beim Erscheinen selbst ins Bild** und fokussiert ihre Überschrift (`scroll-mt-24` – der sticky Header verdeckt sie sonst); Reihenfolge = Terminnotiz → Mailvorschau → **Petrol-Band mit dem wiederverwendeten `SchedulerEmbed`** (Click-to-Load, `cal.eu`) → Verlauf in einem `<details>`. `/kontakt` bleibt als kleiner Textlink darunter – und ist der **ganze** CTA, wenn `NEXT_PUBLIC_CAL_LINK` fehlt (der Vercel-**Preview**-Pfad: dort ist das Band nie zu sehen). ⚠️ Der Verlauf klappt **automatisch auf, wenn keine Terminnotiz zustande kam** – im Fail-safe-Zustand ist er der einzige verbliebene Beleg über dem Ask; wer das `open`-Attribut entfernt, nimmt dem schwächsten Zustand seinen Beweis. Spec/Plan: `docs/superpowers/{specs,plans}/2026-08-01-demo-reveal-buchung*`. Security posture (SSRF resolve-then-pin, denial-of-wallet breaker, no-PII logging, prompt-injection containment) is documented in `docs/superpowers/specs/2026-07-02-termin-quelle-interaktiv-demo-design.md` and guarded by `logging.test.ts`.
...
> - **2026-08-06** — KI-Bildwelt geprüft (Art. 50 Abs. 4a: kein Label im Bild nötig), Transparenzsatz auf `/ueber-mich` unter dem ersten Beat + Test-Guard; Auslöser-Regel von „in Werbung“ auf „erkennbar realer Bezug“ korrigiert.
...
> - **2026-07-29** — Ratgeber-Cluster **live**: 9 Artikel veröffentlicht (3 → 12), 9 Cover, Cross-Article-Pass gegen Serien-Masche.
