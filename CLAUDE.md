...
> - **2026-06-23 — full-bleed page heroes + serif standfirst** (deployed): new **`PageHero`** opens Über-mich/Leistungen/FAQ/Ratgeber (full-bleed image + overlaid title + a **Fraunces serif standfirst lead with a petrol drop cap**, both responsive); **Kontakt/Newsletter deliberately keep `PageIntro`** (form-on-water anchor; cool-on-cool risk). New **hi-res FAQ + Ratgeber images** (2560w, replacing the ~1376w originals); **Leistungen + Über-mich keep current images**. `image_prompt.md` gained a *Full-bleed page heroes* prompt section. Commit 3dd9070.
> - **2026-06-23 — Cal.com scheduler LIVE + Kontakt/Newsletter polish** (deployed): `SchedulerEmbed` restyled on-dark **above** the Kontakt amber form card, pinned to the **EU origin `cal.eu`** (default cal.com 404s the iframe; loads only on click). Kontakt ripple banner now **closes on papier** (caption + „Den Rest bringe ich ins Fließen.“ frame it in serif italic) with symmetric visible padding (Footer's global mt-24/96px absorbed by a smaller `pb`). Both forms: eased the `WaterSection` top pull (`-mt-24/-32`→`-mt-12/-16`) for breathing room under the intro. Commits 0bf07c4…3905213.
