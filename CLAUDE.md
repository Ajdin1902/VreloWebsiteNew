...
All six roadmap phases plus the post-Phase-6 polish are merged and deployed. Since launch the work has been **copy + design refinement** (see *Next session* / *Changelog* below). ⚠ The **2026-06-15 landing-page copy polish is in the working tree — not yet committed/deployed.** The main blocker to go-live is **owner cutover** (domain, env vars, legal sign-off, Resend) — see the TODO blocks below.
...
- **Hero:** warm *Merak* "work done" photo (`/images/hero-merak.webp`, `next/image fill priority` = LCP) with gentle CSS life — a slow ambient zoom (`.hero-scene-img` / `hero-pan`) + 5 drifting dust motes (`.hero-dust` / `dust-float`); decorative `alt=""`, all motion-safe-gated. Hero text reveal is **H1 transform-only** so LCP paints immediately. *(The `RippleImage` WebGL panel moved off the hero onto the `WasIchBaue` drop-on-water image, where ripples fit the „saubere Quelle“ theme.)*
...
1. **Align the homepage sections with the new hero** — the 2026-06-15 pass only retouched a few sections; sweep the remaining section H2s/copy (`Problem`/`WasIchBaue`/`Steps`/`Proof`/`MerakClose`) into one consistent quantified pain→relief voice that flows from the hero (hours saved → calm). Plan in `Knowledge/marketing/LANDING_PAGE.md`.
2. **Legal copy** — replace every `[Platzhalter]` in `src/lib/legal/{impressum,datenschutz}.ts` with real, verified copy (founder/lawyer); also sweep its em-dashes → en-dashes (deferred from the route-page sweep).
3. **Design-skill pass on the remaining subpages** — Leistungen/Über-mich/FAQ + **Ratgeber (index + articles)** are **done**; remaining: **Kontakt, Newsletter, legal** (taste → high-end-visual-design → impeccable, redesign-preserve; browser-verify 1440/390 + AA).
...
5. **Cleanup (optional):** `/video/hero-quelle.jpg` is now unused (the planned move into `WasIchBaue` was dropped — its existing `was-ich-baue.webp` is the cleaner, on-aspect image, and now carries the ripple effect). Remove or keep-for-reuse — decide later.
...
> - **2026-06-15 — landing-page copy + hero image** *(in working tree, not yet committed/deployed)*: pain-first **Hero** copy (see Key decisions); primary CTA → „Ruhe gewinnen“; `WasIchBaue` clearer service H2 + „nichts lernen/warten“ objection cue; `Proof` = trust-by-logic until real Referenzen; `GeschichteTeaser` trimmed to a mention + link. **Hero image** swapped to a warm *Merak* "work done" photo (`hero-merak.webp`) with a slow CSS zoom + drifting dust motes; `RippleImage` retired from the hero. Built via Hormozi `landing-page-copy`+`hormozi-hooks` (brand-filtered) → outputs in `Knowledge/marketing/{LANDING_PAGE,HOOKS}.md`.
> - **2026-06-08** — Ratgeber live (3 seeds `draft:false`); „Flickenteppich“→„Durcheinander“ rename (slug `durcheinander-oder-saubere-quelle`); article reading surface `lesepapier #ece3d2` + `text-lg` (see Gotchas); Ratgeber copy/design polish.
> - **2026-06-07** — real copy across Über-mich / Leistungen (**7 services** in `src/lib/leistungen.ts` + `MehrMoeglich`) / FAQ; per-page `ClosingCta`; `withBrandWords`; `stumm` darkened for AA; Ratgeber per-article covers.
...
- Hero image = warm, concrete *Merak* "work done" scene (static photo + slow CSS zoom + dust motes; decided & wired 2026-06-15, replacing the old deep-water `RippleImage`). Rest of site = calm Papier/petrol sections; page ends on the *Merak*-Effekt. Why warm-hero: it shows the payoff (a calm, finished day), the one deliberate break from the abstract-water world (still no people).
- **Hero copy = quantified pain→relief** (2026-06-15, via `hormozi-hooks`+`brainstorming`): H1 „Wiederkehrende Aufgaben kosten dich jede Woche mehrere Stunden. Ich gebe sie dir zurück.“ + a service sub landing on Ruhe/Kopf (the H1 owns the hours). Why: leads on the concrete hours saved — a stronger, clearer hook than the retired poetic „Vrelo errichtet die Quelle…“; the *Vrelo*/*Merak* duality now lives in `MerakClose`, not the H1. Use honest quantities („mehrere Stunden“) — no invented precise figures.
- **Primary CTA = „Ruhe gewinnen“** (2026-06-15) — the `CTAButton` *default*, so it's site-wide (Header/MobileNav/`ClosingCta`/Hero/`MerakClose`; asserted in `CTAButton.test.tsx`). Why: benefit-led (the calm *Merak* payoff), pairs with the hero sub „Ruhe und einen freien Kopf“. Kept calm — no exclamation/hype. (Superseded the action-clarity „Unverbindlich kennenlernen“ and the earlier poetic „Quelle erkunden“.)
