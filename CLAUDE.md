...
> Homepage copy + hero are **live** (incl. the 2026-06-16 hero-headline readability revision). The end-of-page section order is a parked decision (Ideas.md #8).
...
> - **2026-06-16 — hero headline v2 (readability)**: an impeccable critique found the prior H1 ran to **6 lines** at every breakpoint. Shortened to a single pain line „Manuelle Aufgaben rauben dir die Zeit.“; sub now ends on „jede Woche Stunden zurück“; CTA default realigned „Ruhe gewinnen“ → „Zeit zurückgewinnen“ (matches the time theme). Hero + CTA decisions updated below.
> - **2026-06-15 — landing-page polish**: full homepage copy + image pass via Hormozi `landing-page-copy`/`hormozi-hooks` (brand-filtered; outputs in `Knowledge/marketing/{LANDING_PAGE,HOOKS}.md`). Hero → pain/hours H1 + warm *Merak* "work done" photo (`hero-merak.webp`) with CSS zoom+dust (`RippleImage` moved to `WasIchBaue`); `Problem` expanded to a 4-task list; `WasIchBaue` eyebrow dropped + ripple; `Steps` low-effort lead-in; `Proof` trust-by-logic; `GeschichteTeaser` trimmed to mention+link. End-of-page order deferred (Ideas.md #8).
