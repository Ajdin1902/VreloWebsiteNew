...
1. **Real copy + subpage review** — go page by page and replace placeholders / verify drafts. **Über-mich is done** (real founder copy shipped — see „Done 2026-06-07 — Über-mich“ below). Remaining: the Leistungen + FAQ German drafts (`src/lib/{leistungen,faq}.ts`), the 3 Ratgeber seed articles, and the legal `[Platzhalter]` (`src/lib/legal/*`). While verifying Leistungen/FAQ, also **sweep em-dashes „—“ → en-dash „ – “** in the page leads + `ClosingCta` (homepage was done; subpages weren't). (Supersedes the "Founder copy" item in Open todos.)
