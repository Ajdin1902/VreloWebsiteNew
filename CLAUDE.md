...
- **Homepage narrative reorder — DONE, merged + deployed live** (merge `27808a8`). Reordered the homepage so the offering reads as one unit and the brand story closes the page: **Hero → Problem → Was ich baue → So läuft's ab → Proof → Geschichte → Merak** (`GeschichteTeaser` moved out from between `WasIchBaue` and `Steps` to just before `MerakClose`). „Was ich baue" + „So läuft's ab" now form **one continuous petrol block** with a hairline `border-t border-gletscher/15` seam on `Steps` (two ideas, one substance); the warm **Geschichte → Merak** pair is the emotional finale — the flat-warm Quelle story crescendos into Merak's sunset glow, and the **Vrelo**/**Merak** beats bookend the close. Emotional temperature now rises toward the CTA (deepwater → paper → petrol → paper → warm → sunset). JSX-only change; gate green: 198 tests · tsc · lint · build. (Possible live nudge: divider weight `/15`↔`/20`.)
...
**All planned build work is merged + deployed** — Phase 6, the hero refinement (real drop-footage frame + centered warm-bloom drop), the motion/interaction polish (CTA sheen+lift, hero text reveal-on-load, two-way homepage scroll-reveal), and the homepage narrative reorder (Geschichte → warm finale). The only remaining work is **owner cutover** (domain, env vars, legal sign-off, Resend) — see the TODO blocks below. No code phase is queued.
