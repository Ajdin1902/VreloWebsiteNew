...
> - **2026-06-27 — Lead-Reaktions-Check (`/lead-check`)** (feature merged & live; design polish on `fix/lead-check-polish`): interactive `noindex` lead-magnet — full detail in the *design system* entry above. Pure `leadCheck.ts` model (TDD) + client wizard + optional-email Server Action (server-recompute); linked from the Termin-Quelle block on /leistungen; polish = image `PageHero` (banner like the other subpages) over a cool-band/card-depth wizard panel. 280 tests green, browser-verified 1440/390.
