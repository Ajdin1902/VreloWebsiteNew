...
2. **Design-skill pass on the remaining subpages** — Leistungen/Über-mich/FAQ/Kontakt/**Ratgeber** are **done**; remaining: **Newsletter, legal** (taste → high-end-visual-design → impeccable, redesign-preserve; browser-verify 1440/390 + AA).
...
> - **2026-06-20 — Ratgeber editorial pass**: article page → title+meta+lede first, slim **21:9** cover below the header, **drop-cap** (CSS `.article-body > p:first-of-type::first-letter` in `globals.css` — needs the `.article-body` wrapper), centered **pull-quote** replacing the side-stripe blockquote (Prose `[&_p]:` variants override the mapped inner `<p>`), „← Zurück zum Ratgeber“ back-link; index → thumbnail-less typographic list, intro banner dropped. Spec/plan: `docs/superpowers/{specs,plans}/2026-06-20-ratgeber-editorial*`.
> - **2026-06-20 — homepage copy: `WasIchBaue` sub**: reworked the sub to carry the *accountable-person + never-learn/maintain* differentiator instead of re-stating the Hero's promise; „maßgeschneiderte Automatisierungen“ now has one home (the Hero), and „Kleinkram“ sits only in the Problem H2. Pick validated via the **Hormozi Value Equation** (see *Key decisions* + HQ [CLAUDE.md](../CLAUDE.md) §8).
> - **2026-06-20 — Über-mich, eyebrows & Kontakt banner**: Über-mich copy pass (intro → founder one-liner ending „…und Kaffeeliebhaber“; de-duped „Stress“ → quelle „Hektik“ / merak „Reibung“, kept in ripples + fluss; `ClosingCta` drops the double-„auch …?“ cadence; „Kleinkram“ once, in ripples; Merak stays a felt result, not a thing to „erreichen“) + banner swapped to the Geschichte spring (`aspect-[3/2]`). Removed the decorative nav-repeat eyebrows above headlines — kept „Rechtliches“, „Vertrauen“, FAQ category headings, article meta/tags, OG labels. `RippleImage` (retired hero ripple) reused on the Kontakt banner.
