...
6. **End-stage design pass** — the `RippleImage` WebGL water panel (degrades to a still on reduced-motion / no-WebGL; now orphaned by the centered-spine pass, see *Next session → Cleanup*); **`BrandLockup`** logo in Header + Footer; full-screen **`MobileNav`** drawer; petrol palette rhythm (`WasIchBaue` + `Steps` are `tone="petrol"`; *tiefes-wasser* reserved for Hero + Footer); favicon `src/app/icon.svg`.
...
2. **Design-skill pass on the remaining subpages** — Leistungen/Über-mich/FAQ + **Ratgeber** + **Kontakt** are **done**; remaining: **Newsletter, legal** (taste → high-end-visual-design → impeccable, redesign-preserve; browser-verify 1440/390 + AA). *(Kontakt shipped 2026-06-19 — form in a dark deep-water card on papier + redesigned confirmation; spec/plan in `docs/superpowers/{specs,plans}/2026-06-19-kontakt-redesign*`.)*
...
> - **2026-06-16 — centered-spine layout (impeccable `live`)**: re-laid the whole homepage onto a centered spine; Hero → **full-bleed flowing-water photo + centered overlay** (`hero-flow.webp`, `.hero-overlay-scrim`); `Was ich baue` dropped its image (chips carry it); **`GeschichteTeaser` removed** (story on `/ueber-mich`). Details in *Key decisions* + *design system*.
> - **2026-06-15/16 — landing-page polish + hero headline**: Hormozi-filtered copy/image pass (outputs in `Knowledge/marketing/{LANDING_PAGE,HOOKS}.md`); pain/hours H1 + sub, CTA → „Zeit zurückgewinnen“; `Problem` 4-task list; `Steps` lead-in; `Proof` trust-by-logic.
> - **Earlier (2026-06-07/08)** — Ratgeber went live (renamed „Flickenteppich“→„Durcheinander“, slug `durcheinander-oder-saubere-quelle`; reading surface `lesepapier #ece3d2` + `text-lg`, see Gotchas); real copy across Über-mich / Leistungen (**7 services** + `MehrMoeglich`) / FAQ; per-page `ClosingCta`; `withBrandWords`; `stumm` darkened for AA.
