...
3. **Cleanup (optional):** orphaned (unused) assets — `/images/was-ich-baue.webp`, `/video/hero-quelle.jpg`, and `/images/faq-banner.webp` (FAQ banner removed 2026-06-21). Remove or keep-for-reuse — decide later.
...
> - **2026-06-23 — Cal.com scheduler re-added** (branch `feat/cal-scheduler`, **pending merge+deploy**): `SchedulerEmbed` restyled on-dark, rendered **above** the Kontakt amber form card; pinned to the **EU origin `cal.eu`** (default cal.com would 404 the booking iframe), open-state fixed to a sized papier surface. `NEXT_PUBLIC_CAL_LINK` set in Vercel. Browser-verified the live „Erstgespräch“ calendar at 1440/390. Commit 0bf07c4.
...
> - **2026-06-21 — Newsletter live + redesign**: live on `vercel.app` (double opt-in → Resend **Segment**; Audiences→Segments migration — see *Gotchas*); opt-in email gained greeting/„Bis bald, Ajdin von Vrelo“ sign-off + brand one-liner; **Newsletter** added to nav (**Kontakt-last**); cadence **weekly**; `/bestaetigt` → `RippleImage` payoff; FAQ banner dropped. Commits c77cc75…89a40c1.
...
> - **2026-06-20 — Ratgeber editorial + homepage/Über-mich copy**: article redesign (title+meta+lede, slim 21:9 cover, CSS **drop-cap** needs the `.article-body` wrapper, pull-quote, typographic index; spec/plan `…2026-06-20-ratgeber-editorial*`); `WasIchBaue` sub carries the accountable-person/never-maintain differentiator (Hero owns „maßgeschneiderte Automatisierungen“, „Kleinkram“ only in Problem H2); Über-mich de-duped „Stress“ + dropped nav-repeat eyebrows (kept „Rechtliches“/„Vertrauen“/FAQ/meta/OG); `RippleImage` on the Kontakt banner.
...
**Cal.com scheduler — built + env set, pending deploy.** Re-added above the form on branch `feat/cal-scheduler`; `NEXT_PUBLIC_CAL_LINK = ajdin19/vrelo-kennenlernen` set in Vercel (**Production + Development**; **Preview** still unset — the CLI prompt blocked it, add via dashboard if branch previews should show the calendar). Goes live on **merge → prod redeploy** (founder action); until deployed, prod still shows „Online-Terminbuchung folgt in Kürze“ (the `calLink()` env gate).
