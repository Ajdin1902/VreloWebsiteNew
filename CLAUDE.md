...
> - **2026-06-22 — dark reading-rhythm + honey form cards** (deployed): gave Über-mich/Leistungen/FAQ a petrol/papier light-dark rhythm; homepage Problem → `card-depth` panel, Proof → value cards; Newsletter/Kontakt forms moved into a new reusable **`WaterSection`** (petrol water) and their cards recolored navy → **honig** with on-light text (teal+gold). Added `--color-signal-tief` (on-light error) + `lightLinkClass`; `onDark` variants on StoryBeat/LeistungDetail/FaqItem. Specs/plans `docs/superpowers/{specs,plans}/2026-06-22-*`. Commits 3a75f0e…4003878.
> - **2026-06-21 — Newsletter live + redesign**: went **live** on the `vercel.app` domain (double opt-in → Resend **Segment**). Migrated off Resend's deprecated Audiences API to `contacts.create({ segments:[{ id }] })`; env `NEWSLETTER_AUDIENCE_ID`→`NEWSLETTER_SEGMENT_ID`. Page → Kontakt-style dark card (full `NewsletterForm`; footer `compact` untouched); success → `NewsletterSuccess` (centered thank-you + static water banner); opt-in email gained a greeting, „Bis bald, Ajdin von Vrelo“ sign-off + a brand one-liner. **Newsletter** added to the top nav (header/drawer/footer; nav gap → `gap-5 lg:gap-7` so 6 links + CTA don't crowd at `md`). Confirmation page `/bestaetigt` success → `RippleImage` payoff banner; cadence set to **weekly** („Jede Woche“/„wöchentlich“). Nav order finalized **Kontakt-last**; dropped the FAQ banner image (accordion now flows straight into the CTA). Commits c77cc75…89a40c1.
> - **2026-06-21 — intro-gap + divider cleanup**: tightened the doubled `PageIntro`→`Section` gap and removed the `border-faden` divider lines across Kontakt/FAQ/Über-mich/Leistungen (see *Gotchas → Stacked Sections*). Commits fd34505/3347bc2/dd894e2, all deployed.
> - **2026-06-20 — Ratgeber editorial pass**: article page → title+meta+lede first, slim 21:9 cover, **drop-cap** (CSS `.article-body > p:first-of-type::first-letter` — needs the `.article-body` wrapper), centered pull-quote, back-link; index → typographic list. Spec/plan: `docs/superpowers/{specs,plans}/2026-06-20-ratgeber-editorial*`.
> - **2026-06-20 — homepage `WasIchBaue` sub** reworked to carry the accountable-person/never-maintain differentiator (Hero owns „maßgeschneiderte Automatisierungen“; „Kleinkram“ only in the Problem H2), validated via the Hormozi Value Equation.
> - **2026-06-20 — Über-mich copy + eyebrow removal + Kontakt banner**: de-duped „Stress“, dropped the decorative nav-repeat eyebrows above headlines (kept „Rechtliches“/„Vertrauen“/FAQ/meta/OG), `RippleImage` reused on the Kontakt banner.
> - **2026-06-19 — Kontakt redesign**: form-only page; contact form on a dark `tiefes-wasser` card + `ContactSuccess` confirmation; on-dark `signal` error token; scheduler dropped (component kept). Spec/plan: `docs/superpowers/{specs,plans}/2026-06-19-kontakt-redesign*`.
...
- **Form-error tokens:** **`signal` `#f0a79a`** is the **on-dark** error coral (8.02:1 on `tiefes-wasser`); **`signal-tief` `#8f3526`** (deep brick red) is its **on-light** counterpart for the **honig honey** form cards (AA on honig + papier). The `ContactForm` + full `NewsletterForm` cards are honey now → they use `signal-tief` + the on-light **`lightLinkClass`** (petrol). The **`compact` footer `NewsletterForm`** stays on the dark footer (amber button, `ember` error). On-dark inline links still use **`darkLinkClass`** (gletscher) — both link helpers in `onDarkLink.ts`. Founder-confirm the red hues like `ember`/`stumm`.
