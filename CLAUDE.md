...
> - **2026-06-21 — Newsletter live + redesign**: went **live** on the `vercel.app` domain (double opt-in → Resend **Segment**). Migrated off Resend's deprecated Audiences API to `contacts.create({ segments:[{ id }] })`; env `NEWSLETTER_AUDIENCE_ID`→`NEWSLETTER_SEGMENT_ID`. Page → Kontakt-style dark card (full `NewsletterForm`; footer `compact` untouched); success → `NewsletterSuccess` (centered thank-you + water banner); opt-in email gained a greeting, „Bis bald, Ajdin von Vrelo“ sign-off + a brand one-liner. Commits c77cc75/d2f8b7e/c7f1b93.
