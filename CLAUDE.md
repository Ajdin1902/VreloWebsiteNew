...
> - **2026-06-20 — eyebrow/kicker cleanup**: removed the decorative nav-repeat eyebrows above headlines (Leistungen/Kontakt/FAQ/Ratgeber index + article header/Newsletter). Kept the ones that add info: „Rechtliches“ (legal grouping), „Vertrauen“ (Referenzen), FAQ category headings, article meta/tags, OG-image labels. `PageIntro` keeps its optional `eyebrow` prop (Legal still uses it).
