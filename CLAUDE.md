...
> Homepage is **live** on a centered spine with a full-bleed flowing-water hero (`hero-flow.webp`, Direction C), 2026-06-16. **In flight — built + tested (217 green), committed locally, NOT pushed:** the Problem + Proof depth pass — Problem task list now in a `card-depth` panel; Proof rebuilt into 4 value cards (incl. a 3+yr-experience proof point) + an honest references footnote. Commits 7e62e37/715019a; spec+plan at `docs/superpowers/{specs,plans}/2026-06-22-homepage-problem-proof-depth*`. **Open:** local review → prod push; verify the panel/cards lift off papier (else add `ring-1 ring-tiefes-wasser/5`).
...
2. **Design-skill pass on the remaining subpages** — Leistungen/Über-mich/FAQ/Kontakt/Ratgeber/**Newsletter** are **done**; remaining: **legal** (taste → high-end-visual-design → impeccable; browser-verify 1440/390 + AA).
...
6. **Darker tones across subpages (maybe)** — explore a bit more depth on **Über-mich / FAQ / Leistungen** (currently mostly papier): more tinted/petrol/`tiefes-wasser` sections for richer rhythm. Keep AA (re-check text tokens on any darker surface — see *Gotchas*). Taste-led, next session.
