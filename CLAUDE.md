...
> - **2026-06-24 — closing-CTA pop + lighter heroes** (deployed): `ClosingCta` button → new navy **`CTAButton variant="inverse"`** for contrast on the warm band (heading keeps its ember); `PageHero` heroes `quality={65}` + `next.config` `qualities=[65,75]` (the slow-hero was the optimizer cold-cache, not resolution — see *Gotchas*). Commits 96918d8, e33d654.
