...
- **Full-bleed page heroes (`PageHero`):** Über-mich/Leistungen/FAQ/Ratgeber/lead-check/demo/makler open with a full-bleed image, title on `.hero-overlay-scrim`, lead on paper below as a Fraunces serif standfirst with a petrol drop-cap. Heroes render at `quality={65}`. *(The `actions` and `priority` props were removed 2026-08-08 — this doc claimed `actions` was „used by `/makler`", but no call site ever passed either; `/makler`'s CTA lives in the focus header.)* **Kontakt + Newsletter keep `PageIntro` (no hero)** — a cool hero stacked on the cool form read as water-on-water.
...
- **Focus routes (`ChromeGate`):** routes in `focusRoutes` (`src/lib/nav.ts`) render **without the site Header/Footer** — for single-purpose outreach pages where every nav link is an exit before the CTA. `ChromeGate` is a client component reading `usePathname()`; on a focus route it **swaps in** `FocusHeader`/`FocusFooter` (`src/components/focus/`) instead of returning `null`. **`Header`/`Footer` must be passed as children, never as a prop** (RSC boundary); the `slot` prop tells the gate which half it stands in for. Chosen over route groups to avoid moving twelve route folders for zero functional gain.
  ...
  - **`/makler` and `/lead-check`** are focus routes. `/lead-check` joined on **2026-08-08** (Rams audit): it carried **19** navigational exits and a **second, competing email capture** (the footer `NewsletterForm`) around a six-question task — on the page the Tier-A outreach links to. **`/demo` deliberately keeps full chrome** — it is a sandbox people are meant to wander out of.
...
> - **2026-08-08** – Dieter-Rams-Audit von `/makler` + `/lead-check` (**19/30**, Artefakte unter `docs/superpowers/DESIGN-IS-2026-08-08/`) und dessen Umsetzung: €-Modell entschärft (`RECOVERY_RATE`), Einwilligung bei der Mail-Erfassung, Quiz-Bedienbarkeit (Fokus, Gruppen-Semantik, Live-Region, 3,77:1-Ränder), Claim-Rewrites inkl. der Bedrock-Offenlegung, `/lead-check` als Focus-Route, Chrome in den Root-Layout gezogen (Landmarks). Details in den Bullets oben.
