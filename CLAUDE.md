...
> - **2026-06-27 — Lead-Reaktions-Check (`/lead-check`)** (merged & live): interactive `noindex` lead-magnet — full detail in the *design system* entry above. Pure `leadCheck.ts` model (TDD) + client wizard + optional-email Server Action (server-recompute); image `PageHero` over a cool-band/card-depth wizard; linked from the Termin-Quelle block on /leistungen.
...
**Newsletter — ✅ LIVE (2026-06-21)** on the current `vercel.app` domain. `NEWSLETTER_SECRET` + `NEWSLETTER_SEGMENT_ID` set in Vercel (Production); double opt-in confirmed end-to-end (contact lands in the Resend **Segment**). Runbook: [docs/newsletter-golive-runbook.md](docs/newsletter-golive-runbook.md). **At the `vrelo-ki.de` cutover:** set `NEXT_PUBLIC_SITE_URL` and **keep the same `NEWSLETTER_SECRET`** (subscribers + pending opt-in links carry over). **Sending issues is built (2026-06-27):** author `content/newsletter/*.md`, then `npm run newsletter` (preview/test/broadcast to the segment). See the `sending-newsletter` skill.
