...
> - **Article-only reading surface** — articles use a deeper **`lesepapier #ece3d2`** via **`Section tone="reading"`**; site-wide papier stays `#f4efe6` (deliberate scoped split, chosen via a live A/B — see Gotchas for AA limits). Article body bumped to **`text-lg` (18px)** (`Prose.tsx`); index list left-aligned to the headline. Accepted tradeoff: faint header seam on articles (header is papier/90 + blur).
